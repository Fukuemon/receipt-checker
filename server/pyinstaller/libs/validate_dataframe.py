...
from server.libs.constant import SERVICE_CODE_VALID_TIME_RANGES, MERGE_KEY_COLUMNS, CHECK_COLUMNS
...
def validate_end_time(service, start_time, end_time):
    ...
    :return: (bool, str) チェック結果と有効範囲の文字列
    """
    if '基本療養費' in service or '難病等複数回訪問' in service:
        start_minutes, end_minutes = SERVICE_CODE_VALID_TIME_RANGES[key]

    # 通常のサービスコードに基づく時間範囲チェック
    if service not in SERVICE_CODE_VALID_TIME_RANGES:
        return False, ""
    start_minutes, end_minutes = SERVICE_CODE_VALID_TIME_RANGES[service]
...
def check_columns(merged_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in CHECK_COLUMNS:
        if column == 'サービス内容':
            merged_df.loc[:, column + '_match'] = merged_df.apply(
            ...
            merged_df.loc[:, column + '_match'] = merged_df[column + '_カレンダー'] == merged_df[column + '_Ibow']
            merged_df = validate_service_times(merged_df)
        elif column == "終了時間":
            merged_df.loc[:, '終了時間_match'], merged_df.loc[:, '終了時間_有効範囲'] = zip(*merged_df.apply(
